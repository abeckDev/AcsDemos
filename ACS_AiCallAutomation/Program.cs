...
{
    Console.WriteLine($" 1 WebSocket request received: {context.Request.Path}");
    if (context.Request.Path == "/ws")
    {
        Console.WriteLine($" 2 WebSocket request received: {context.Request.Path}");
        if (context.WebSockets.IsWebSocketRequest)
        {
            Console.WriteLine($" 3 WebSocket request received: {context.Request.Path}");
            try
            {
